(function() {
    ...
    function submitForm(element) {
        ...
        // Method 2: Look for nearby submit buttons
        let submitButton = element.parentElement?.querySelector('button[type="submit"], input[type="submit"]');
        ...
        if (!submitButton) {
            // Look one level up
            const parentButtons = element.parentElement?.parentElement?.querySelectorAll('button') || [];
            submitButton = Array.from(parentButtons).find(btn => 
                !btn.type || btn.type === 'submit' || 
                btn.textContent?.toLowerCase().includes('send') ||
                btn.textContent?.toLowerCase().includes('submit')
            );
        ...
        // Method 3: Dispatch Enter keypress to let the site handle it
        ...
        console.log('Dispatched Enter keypress event');
