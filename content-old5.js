(function() {
    ...
    function shouldReverseLogic(element) {
        // Check if the site expects Shift+Enter to submit (opposite of our default)
        const placeholder = element.placeholder?.toLowerCase() || '';
        const ariaLabel = element.getAttribute('aria-label')?.toLowerCase() || '';
        
        // Look for patterns that suggest Shift+Enter should submit
        ...
        const shouldReverse = reversePatterns.some(pattern => 
            placeholder.includes(pattern) || ariaLabel.includes(pattern)
        );
        
        if (shouldReverse) {
            console.log('Detected site expects Shift+Enter to submit, reversing logic');
            return true;
        }
    ...
    function handleKeyDown(event) {
        ...
        // Check if this site has reversed logic (Shift+Enter to submit)
        const hasReversedLogic = shouldReverseLogic(element);
        
        if (hasReversedLogic) {
            // Site expects Shift+Enter to submit, Enter to newline
            if (event.shiftKey) {
                // Shift+Enter: Submit (site's expectation)
                console.log('Reversed logic: Shift+Enter submitting');
                event.preventDefault();
                event.stopPropagation();
                submitForm(element);
            } else {
                // Plain Enter: Insert newline (site's expectation)
                console.log('Reversed logic: Plain Enter inserting newline');
                event.preventDefault();
                event.stopPropagation();
        ...
        // Normal logic (our default behavior)
        if (event.shiftKey) {
